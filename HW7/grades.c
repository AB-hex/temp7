...
int clone_course(void *element, void **output){
    ...
    strcpy(out_course->name , new_course->name);
...
void destroy_student (void *element) {
    ...
    for(int i=0; i<list_size(Student->courses);++i){
        destroy_course(Student->courses[i]);
    }
...
struct grades* grades_init(){
    ...
    element_clone_t clone_element;
    element_destroy_t destroy_element;
...
void grades_destroy(struct grades *grades){
        ...
        for(int i=0; i<list_size(grades->students_list);++i){
            destroy_student(grades->students_list[i]);
        }
...
int grades_add_student(struct grades *grades, const char *name, int id){
    ...
    return list_push_back(grades->students_list , &student );
};
...
int grades_add_grade(struct grades *grades,
    ...
    return list_push_back(s->courses , &course);
};
